...
def analyse_step2(files, repo_name):
    ...
    prompt = (
        ...
        "- EXTERNAL SCRIPTS: if app.js/main.js referenced via <script src=> tag — JS is present externally, NEVER flag as missing.\\n"
        ...
        "- MINIFIED CODE: truncated or minified CSS/JS in HTML is normal — NEVER flag as incomplete or broken.\\n\\n"
...
def analyse_step4(repo_name, built_with, findings_summary):
    ...
    prompt = (
        ...
        "FIX_1_TITLE: most urgent fix based on actual findings (or 'No critical fixes needed' if score is A)\n"
        "FIX_1_WHY: why it matters in one sentence\n"
        "FIX_1_HOW: 2-3 step fix instruction\n"
        "FIX_1_EFFORT: 5 minutes|30 minutes|1 hour|1 day\n"
        "FIX_1_PROMPT: advice prompt — paste into " + platform + " and ask it to investigate and advise, not blindly change. Begin with: I received a security review flagging this. Please review and advise without making changes yet\n"
        "FIX_2_TITLE: second fix\n"
        "FIX_2_WHY: why\n"
        "FIX_2_HOW: how\n"
        "FIX_2_EFFORT: effort\n"
        "FIX_2_PROMPT: advice prompt for " + platform + " — investigate and advise first, suggest minimal safe change only if confirmed needed\n"
        "FIX_3_TITLE: third fix\n"
        "FIX_3_WHY: why\n"
        "FIX_3_HOW: how\n"
        "FIX_3_EFFORT: effort\n"
        "FIX_3_PROMPT: advice prompt for " + platform + " — investigate and advise first, suggest minimal safe change only if confirmed needed\n"
        ...
        "FIX_4_PROMPT: advice prompt for " + platform + " — investigate and advise first, minimal safe change only if needed\\n"
        ...
        "FIX_5_PROMPT: advice prompt for " + platform + " — investigate and advise first, minimal safe change only if needed\\n"

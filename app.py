...
def admin_feedback():
    """Private feedback inbox — protected by ADMIN_KEY env var. Fails closed if unset."""
    admin_key = os.getenv("ADMIN_KEY", "")
    provided = request.args.get("key", "")
    if not admin_key or not _secrets.compare_digest(provided, admin_key):
        return "Not found", 404

    rows = []
    if _HAS_SUPABASE:
        try:
            res = (_sb.table("reports")
                   .select("id,repo,created_at,score,data")
                   .order("created_at", desc=True)
                   .limit(1000)
                   .execute())
            for r in (res.data or []):
                d = r.get("data") or {}
                if "feedback_helpful" not in d:
                    continue
                rows.append({
                    "id": r.get("id", ""),
                    "repo": r.get("repo", "") or "(unknown)",
                    "created_at": (r.get("created_at", "") or "")[:16].replace("T", " "),
                    "helpful": bool(d.get("feedback_helpful")),
                    "comment": d.get("feedback_comment", "") or "",
                    "email": d.get("feedback_email", "") or "",
                    "score": r.get("score") or d.get("health", {}).get("score", "") or "?",
                })
        except Exception as e:
            return f"Error loading feedback: {e}", 500

    # Action items (thumbs down) first, then the rest — both newest-first within each group
    down = [r for r in rows if not r["helpful"]]
    up = [r for r in rows if r["helpful"]]
    ordered = down + up
    n_up = len(up)
    n_down = len(down)
    n_email = len([r for r in rows if r["email"]])
    ...
    body = []
    for r in ordered:
        tone = "#E24B4A" if not r["helpful"] else "#1D9E75"
        bg = "#FCEBEB" if not r["helpful"] else "#fff"
        icon = "&#x1F44E;" if not r["helpful"] else "&#x1F44D;"
        report_link = f'<a href="/report/{esc(r["id"])}" target="_blank" style="color:#534AB7;text-decoration:none">open report &#8599;</a>'
        if r["email"]:
            subj = f"Re: your Verilay feedback on {r['repo']}"
            mailto = f'<a href="mailto:{esc(r["email"])}?subject={esc(subj)}" style="color:#534AB7;font-weight:600;text-decoration:none">{esc(r["email"])}</a>'
        else:
            mailto = '<span style="color:#9b9996">no email</span>'
        comment = esc(r["comment"]) if r["comment"] else '<span style="color:#9b9996">no comment</span>'
        body.append(
            f'<tr style="background:{bg}">'
            f'<td style="padding:10px;border-bottom:1px solid #eee;font-size:18px">{icon}</td>'
            f'<td style="padding:10px;border-bottom:1px solid #eee;white-space:nowrap;color:#6b6966;font-size:12px">{esc(r["created_at"])}</td>'
            f'<td style="padding:10px;border-bottom:1px solid #eee;font-weight:600">{esc(r["repo"])}<br><span style="font-weight:400;color:#9b9996;font-size:12px">score {esc(r["score"])}</span></td>'
            f'<td style="padding:10px;border-bottom:1px solid #eee;font-size:13px;color:{tone}">{comment}</td>'
            f'<td style="padding:10px;border-bottom:1px solid #eee;font-size:13px">{mailto}</td>'
            f'<td style="padding:10px;border-bottom:1px solid #eee;font-size:12px">{report_link}</td>'
            f'</tr>'
        )
    if not ordered:
        body.append('<tr><td colspan="6" style="padding:20px;text-align:center;color:#9b9996">No feedback yet.</td></tr>')

    html = (
        '<!doctype html><html><head><meta charset="utf-8">'
        '<meta name="viewport" content="width=device-width,initial-scale=1">'
        '<title>Feedback inbox - Verilay</title></head>'
        '<body style="font-family:-apple-system,BlinkMacSystemFont,Segoe UI,sans-serif;max-width:1000px;margin:0 auto;padding:24px;color:#1a1917">'
        '<h1 style="font-size:22px;margin-bottom:4px">Feedback inbox</h1>'
        f'<p style="color:#6b6966;font-size:14px;margin-top:0">&#x1F44D; {n_up} helpful &nbsp;&middot;&nbsp; &#x1F44E; {n_down} not helpful &nbsp;&middot;&nbsp; {n_email} left an email. Thumbs-down shown first.</p>'
        '<table style="width:100%;border-collapse:collapse;font-size:14px">'
        '<thead><tr style="text-align:left;color:#6b6966;font-size:12px">'
        '<th style="padding:10px"></th><th style="padding:10px">When</th><th style="padding:10px">App</th>'
        '<th style="padding:10px">Comment</th><th style="padding:10px">Reply to</th><th style="padding:10px"></th>'
        '</tr></thead><tbody>'
        + "".join(body) +
